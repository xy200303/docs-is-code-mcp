# 补充行为记录与实现计划模板

## Meta

- status: done
- source: user-prompt

## 用户原始描述

增强 spec 工具的中文模板和记录能力：done 归档与 checkpoint/review 结果里要能记录系统实际行为，包括业务分支条件、默认参数行为、边界处理和验证结果；active spec 模板里要提供“AI 实现计划/方案”区域，用中文描述 AI 打算如何具体实现系统，包括改动范围、关键文件、数据流、分支处理、默认值和风险确认。更新模板、writer 输出和测试，保持内容简洁可维护。

## TODO

- [x] 增强 spec 工具的中文模板和记录能力：done 归档与 checkpoint/review 结果里要能记录系统实际行为，包括业务分支条件、默认参数行为、边界处理和验证结果；active spec 模板里要提供“AI 实现计划/方案”区域，用中文描述 AI 打算如何具体实现系统，包括改动范围、关键文件、数据流、分支处理、默认值和风险确认。更新模板、writer 输出和测试，保持内容简洁可维护。

## 执行要求

- 开始前必须先调用 `spec_context`，确认当前 TODO 上下文和工程约束。
- AI 必须按未勾选 TODO 从上到下执行。
- 完成任务后把对应项改成 `[x]`。
- 无法完成的任务保持 `[ ]`，并在任务下方写明阻塞原因。

## 工程质量约束

这些规则是强制约束，不是建议。

### Hard Rules

- Fail Fast：尽早校验输入、依赖、前置条件和无效状态。
- 风险先确认：不明确、高影响或高风险决策先问用户。
- 文件注释：新建或重写文件保留顶部注释；复杂边界写为什么，不写废话。
- 禁止在一个文件里混合 UI、业务、数据访问逻辑；禁止在领域层引用 Web / DB 框架。
- 禁止为了模式而模式：不要无故引入接口、工厂、泛型、抽象层。
- 性能与资源：避免不必要高复杂度，不阻塞主线程，不泄露连接、内存或文件句柄。

### Recommended Practices

- KISS + YAGNI：优先最简单可用方案，不预埋未确认复杂度。
- Clean Code：业务意图命名，短函数，低嵌套，DRY，显式行为。
- Human Readable：按线性故事写代码，复杂逻辑拆成有语义的小步骤。
- Clean Architecture + DDD：按业务能力分层，领域规则不依赖框架、DB 或 Web。
- SOLID + SoC：职责单一，关注点分离，组合优于继承，依赖抽象。
- 测试优先：核心逻辑可单测，验证命令和结果必须记录。
- 向后兼容：小步修改，不破坏已有 API、数据和行为契约。
- 成熟库优先：已有成熟方案不手搓；新增依赖先确认必要性。
- 项目结构：按业务语义拆分目录和文件，避免单文件堆砌和目录平铺。
- UI/交互：符合直觉，状态完整，文案简洁，布局清楚。
- Boy Scout Rule：局部顺手清理，不做无关大重构。
- AI + Human：结构清晰、边界明确，便于 AI 修改和人类维护。

## 业务不确定性强制确认

这些规则是硬性约束，不是建议。

- 业务不确定性强制确认：金额、费率、结算、退款、折扣、税费、状态机、并发、幂等、重试、回滚、规则来源不明或角色差异，必须先问清楚。
- 禁止猜业务：不要用常识补规则，不要自行假设边界。
- 澄清格式：说明不清楚之处，给出 2 到 3 种可能解释，等待用户确认。
- 金钱与合规：涉及钱、合规、审计的实现必须有明确来源或产品确认注释。

## Checkpoint

- at: 2026-06-19T18:22:34.533Z
- summary: 增强 spec 模板和记录器：active/spec 模板增加中文 AI 实现计划与实际行为记录区；TODO 模板要求完成后记录分支条件、默认参数和边界处理；checkpoint/review/done 支持 behaviorNotes，并在未提供时显示中文未记录占位。
- note: 实际行为记录字段应使用中文描述已确认行为，尤其是分支条件、默认参数行为、边界处理和验证结果。

### Summary

- 增强 spec 模板和记录器：active/spec 模板增加中文 AI 实现计划与实际行为记录区；TODO 模板要求完成后记录分支条件、默认参数和边界处理；checkpoint/review/done 支持 behaviorNotes，并在未提供时显示中文未记录占位。

### Completed TODOs

- 增强 spec 工具的中文模板和记录能力：done 归档与 checkpoint/review 结果里要能记录系统实际行为，包括业务分支条件、默认参数行为、边界处理和验证结果；active spec 模板里要提供“AI 实现计划/方案”区域，用中文描述 AI 打算如何具体实现系统，包括改动范围、关键文件、数据流、分支处理、默认值和风险确认。更新模板、writer 输出和测试，保持内容简洁可维护。

### Changed Files

- `src/spec/behavior-record.ts`
- `src/spec/checkpoint-writer.ts`
- `src/spec/review-result-writer.ts`
- `src/spec/done-writer.ts`
- `src/spec/types.ts`
- `src/mcp/write-schemas.ts`
- `src/mcp/register-write-tools.ts`
- `src/templates/prompt-documents.ts`
- `src/templates/spec-documents.ts`
- `test/unit.ts`
- `test/smoke.ts`
- `README.md`

### Verification

- passed `bun run build`
- passed `bun test/unit.ts`：测试主体通过；当前沙箱在结束后输出 EPERM 读取用户目录噪声但退出码为 0。
- passed `bun test/smoke.ts`：测试主体通过；当前沙箱在结束后输出 EPERM 读取用户目录噪声但退出码为 0。
- passed `bun run release:check`：契约检查通过；当前沙箱在结束后输出 EPERM 读取用户目录噪声但退出码为 0。
- passed `git diff --check`

### Risks

- behaviorNotes 仍需要模型或用户填写，工具只提供结构和占位，不自行推断业务行为。

### Blockers

- 无

## Done

- doneAt: 2026-06-19T18:22:42.475Z
- note: 已增强中文 AI 实现计划与实际行为记录模板。

## 实际行为记录

- 分支条件：active spec 模板记录 AI 实现计划，checkpoint/review/done 记录完成后的实际行为。
- 默认参数行为：未提供 behaviorNotes 时写入中文“未记录”占位，提醒后续补充。
- 边界处理结果：工具不自行推断业务行为，只保存模型或用户确认后的中文记录。
- 验证结果：构建、单测、烟测、release check 和 diff check 均已通过。
