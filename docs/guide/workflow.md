...
`spec_context` 会输出 guidance 索引和轻量推荐，要求 AI 在需要时读取 `engineering`、`ui-ux`、`spec-writing` 或 `quality-review`。UI/UX guidance 强调事实优先和语境优先：设计前先确认真实定位、用户、核心对象、事实来源和 CTA，不编造指标、客户、邮箱、性能数据或商业定位；官网类任务按企业官网、产品官网、作品集、开源组织或文档站分别设计，只有明确是 OSS/开源组织时才默认使用 GitHub/repo/contribution/license 等结构。
